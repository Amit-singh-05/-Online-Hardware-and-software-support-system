...
public class StaffDaoImpl implements StaffDao {
	...
	public List<Staff> getAllEnginner() throws StaffException {
		...
		try(Connection conn= DBconnect.provideConnection()) {
					...
					PreparedStatement ps= conn.prepareStatement("select * from members_of_organization where category is not null");
	...
	@Override
	public String removeStaff(String runame) throws StaffException {
		String message = "Not Inserted..";
		
		try(Connection conn= DBconnect.provideConnection()) {
			
			PreparedStatement ps= conn.prepareStatement
					("delete from members_of_organization where username = ?");
			...
			if(x > 0)
				message = "Engineer Removed Sucessfully !";
	...
	@Override
	public String change_password(String npassword,String nname) throws StaffException {
		String message = "Password Not changed..";
	try(Connection conn= DBconnect.provideConnection()) {
			
			PreparedStatement ps= conn.prepareStatement("update members_of_organization set password = ? where name = ?");
