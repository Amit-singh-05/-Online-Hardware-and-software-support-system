...
public class SomplainDaoImpl implements SomplainDao {
	...
	@Override
	public List<ComplaintTable> getasindengineer(String s) throws StaffException {
	List<ComplaintTable> complaint3= new ArrayList<>();
			
			try(Connection conn= DBconnect.provideConnection()) {
						
						PreparedStatement ps= conn.prepareStatement("select * from complaint where assigned_engineer = ?");
						ps.setString(1, s);
						ResultSet rs= ps.executeQuery();
	...
	public String updatestatus(String complaint_status,int Complaint_id) throws StaffException {
		String message = "complaint status not updated..";
	...
	@Override
	public List<ComplaintTable> getcomplainattended(String s) throws StaffException {
	List<ComplaintTable> complaint3= new ArrayList<>();
			
			try(Connection conn= DBconnect.provideConnection()) {
						
						PreparedStatement ps= conn.prepareStatement("select * from complaint where assigned_engineer = ? and complaint_status is not null");
						ps.setString(1, s);
						ResultSet rs= ps.executeQuery();
